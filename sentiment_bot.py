...
==========================================
v11 변경 (한국 메인 / 미국 보조 이원 센티먼트):
  - 메인 온도 = 한국 구성요소만: 국내 정량 5종(가중: 고점比14·실현변동성14·이격도12·
    레버리지/인버스10·원달러10 = 60) + AI 조사 20(가용 시) + 개인 체감 20
  - 미국(VIX·CNN F&G·HY 스프레드)은 메인에서 제외, 동일가중 보조 온도로 별도 산출
  - 한-미 온도 괴리를 포인트로 직접 표시 (국지적 패닉/과열 판정 기준 ±1.0점 유지)
...
DIGEST_DIR = os.environ.get("DIGEST_DIR", "digests")     # 다이제스트 파일 디렉토리 (같은 repo)
DIGEST_REPO = os.environ.get("DIGEST_REPO", "")          # 다른 repo면 "owner/repo" 형식
DIGEST_PATH = os.environ.get("DIGEST_PATH", "digests")   # 다른 repo 내 경로
GH_PAT = os.environ.get("GH_PAT", "")                    # 다른 private repo 접근용 토큰
...
PORTFOLIO_FILE = os.path.join(STATE_DIR, "portfolio.json")     # 등록된 포트폴리오
...
AUTO_INDICATORS = [
    # (표시명, 함수, 지역, 가중치) — v11: 한국 메인 온도는 국내 항목만 사용 (가중 60)
    #   미국(글로벌) 3종은 메인 온도에서 제외, 보조 온도로 별도 산출 (동일 가중)
    ("KOSPI 고점대비", score_kospi_drawdown, "국내", 14),
    ("KOSPI 실현변동성", score_kospi_realized_vol, "국내", 14),
    ("KOSPI 120일 이격도", score_kospi_disparity, "국내", 12),
    ("레버리지/인버스 비율", score_lev_inv_ratio, "국내", 10),
    ("원달러 환율", score_usdkrw, "국내", 10),
    ("VIX", score_vix, "글로벌", None),
    ("CNN Fear&Greed", score_cnn_fng, "글로벌", None),
    ("하이일드 스프레드", score_hy_spread, "글로벌", None),
]
...
def postprocess_auto(results, failed, meta=None):
    """채점 결과 후처리 (v11): 동학개미 보정 + 한국 가중평균(메인) + 미국 보조 온도."""
    ...
    if core_avg is not None and core_avg <= 1.5 and scores.get("레버리지/인버스 비율", 0) >= 4:
        ...
        donghak = True
    # 한국 메인: 국내 항목 가중평균 (실패 항목은 가중 재정규화)
    kr = [r for r in results if r["region"] == "국내"]
    kr_wsum = sum(r.get("w") or 0 for r in kr)
    if not kr or not kr_wsum:
        raise RuntimeError("국내 정량 지표 전체 실패 — 한국 센티먼트 산출 불가")
    kr_avg = sum(r["score"] * (r.get("w") or 0) for r in kr) / kr_wsum
    # 미국 보조: 글로벌 항목 동일가중 → 별도 온도
    us = [r["score"] for r in results if r["region"] == "글로벌"]
    us_avg = sum(us) / len(us) if us else None
    us_temp = (us_avg - 1) / 4 * 100 if us_avg is not None else None
    out = {"items": results, "avg": kr_avg, "weight": AUTO_WEIGHT_TOTAL, "failed": failed,
           "scores": scores, "core_kr": core_avg, "donghak": donghak,
           "us_avg": us_avg, "us_temp": us_temp,
           "국내": kr_avg, "글로벌": us_avg,
...
def collect_auto():
    results, failed = [], []
    for name, fn, region, w in AUTO_INDICATORS:
        try:
            s, detail = fn()
            results.append({"name": name, "score": s, "detail": detail,
                            "region": region, "w": w})
...
# ======================================================================
# 리서치 다이제스트 톤 지표 (v12)
#   mason_digest_bot(리서치 에이전트)의 요약 10일치를 분석해 애널리스트 톤 채점.
#   텔레그램 봇은 타 봇 메시지를 읽을 수 없으므로(Bot API 제한) 원천 통합:
#   에이전트가 발송 직후 digests/YYYY-MM-DD.md 로 저장 → 심리봇이 파일을 읽음.
# ======================================================================
DIGEST_DAYS = 10
DIGEST_WEIGHT = 6
...
def collect_digest_item():
    ...
    text = read_digests()
    if not text:
        print("[다이제스트 없음] digests/ 디렉토리 또는 DIGEST_REPO 미구성 — 지표 제외")
        return None
    prompt = (
        f"다음은 최근 {DIGEST_DAYS}일간 한국 증권사 리서치 요약(일자별)이다. 전체 애널리스트 톤을 "
        "1~5점으로 채점하라 (1=극단적 공포, 5=극단적 탐욕).\n"
        "채점 기준: 1=목표가·투자의견 하향 러시, '보수적 접근' 지배 / 2=신중론 우세, 목표가 동결 / "
        "3=혼재·평시 / 4=목표가 상향 우세, 실적 낙관 / 5=경쟁적 목표가 상향, 새 밸류에이션 "
        "프레임으로 랠리 정당화.\n"
        "주의: 국내 증권사는 매수 일변도 문화이므로 '중립 하향'·목표가 하향·리스크 경고의 출현 "
        "자체를 강한 부정 신호로 가중 해석하고, 최근 일자에 더 큰 비중을 두라.\n"
        '응답은 JSON 객체 하나만: {"score": 3, "rationale": "근거 한 줄"}\n\n'
        f"{text}"
    )
    try:
        out = _ai_query_gemini(prompt) if GEMINI_KEY else _ai_query_claude(prompt)
        m = re.search(r"\{.*\}", out.replace("```json", "").replace("```", ""), re.S)
        p = json.loads(m.group(0))
        score = int(p["score"])
        if not (1 <= score <= 5):
            raise ValueError(f"점수 범위 오류: {score}")
        print(f"[다이제스트 톤] {score}점 — {p.get('rationale','')[:80]}")
        return {"name": f"리서치 다이제스트 톤({DIGEST_DAYS}일)", "score": score,
                "weight": DIGEST_WEIGHT, "lagging": False,
                "rationale": str(p.get("rationale", ""))[:120]}
    except Exception as e:
        print(f"[다이제스트 채점 실패 → 지표 제외] {type(e).__name__}: {e}")
        return None


def merge_ai(ai, extra_items):
    """AI 조사 결과에 추가 항목(다이제스트 등)을 병합하고 가중 재계산.
    웹조사 실패 + 다이제스트 성공 같은 부분 가용 상태도 지원."""
    items = (ai["items"] if ai else []) + [x for x in extra_items if x]
    if not items:
        return None
    w = sum(i["weight"] for i in items)
    avg = sum(i["score"] * i["weight"] for i in items) / w
    return {"items": items, "avg": avg, "weight": w}
...
def build_interim(auto, ai):
    lines = ["📊 *정량 지표 채점* (1=공포~5=탐욕)"]
    for region, label in (("국내", "🇰🇷 한국 (메인)"), ("글로벌", "🇺🇸 미국 (보조)")):
        sub = [r for r in auto["items"] if r["region"] == region]
        if not sub:
            continue
        lines.append(f"  [{label}]")
        for r in sub:
            w = f" (w{r['w']})" if r.get("w") else ""
            lines.append(f"  · {r['name']}{w}: *{r['score']}점* — {r['detail']}")
    if auto["failed"]:
        lines.append(f"  ⚠️ 수집 실패(제외): {', '.join(auto['failed'])}")
    kr, us = auto.get("국내"), auto.get("글로벌")
    line = f"  → 한국 정량 {kr:.2f}/5"
    if us is not None:
        line += f" · 미국 {us:.2f}/5"
    lines.append(line)
    if us is not None:
        div = kr - us
        if div <= -1.0:
            lines.append(f"  🚨 *한-미 괴리 {div:+.2f}*: 한국 국지적 패닉. "
                         "글로벌 리스크오프가 아닌 국내 수급(레버리지 청산·ADR 등) 요인 점검 필요")
        elif div >= 1.0:
            lines.append(f"  🚨 *한-미 괴리 {div:+.2f}*: 한국 국지적 과열. "
                         "미국 대비 앞서간 만큼 되돌림 취약")
    if ai:
        lines.append("\n🔍 *2) AI 웹조사 채점*")
        for i in ai["items"]:
        ...
        lines.append("\n_AI 웹조사 미가용 — 커뮤니티·수급·공모주 항목은 이번 산출에서 제외됨_")
    return "\n".join(lines)
...
def build_final(auto, ai, answers):
    ...
    if ai:
        ai_w = ai.get("weight", AI_W)  # v12: 다이제스트 병합 등 부분 가용 시 동적 가중
    ...
    if ai:
        ...
        ai_line = "_AI 조사 미가용 — 한국 정량 60 : 개인 체감 20으로 재정규화 산출_\n"
    us_line = ""
    if auto.get("us_temp") is not None:
        us_name, _ = regime(auto["us_temp"])
        us_line = (f"🇺🇸 미국 보조 온도: *{auto['us_temp']:.0f}* — {us_name}\n"
                   f"한-미 온도 괴리: {temp - auto['us_temp']:+.0f}p\n")
    msg = (
        f"🇰🇷 *한국 시장 심리 온도: {temp:.0f} / 100* (메인)\n"
        f"판정: {name}\n"
        + us_line + "\n"
        f"한국 정량({AUTO_WEIGHT_TOTAL}): {auto['avg']:.2f}/5 · 체감({sub_w}): {sub_avg:.2f}/5\n"
        + ai_line
    ...
    msg += "\n\n━━━━━━━━━━━━\n" + build_portfolio_guide(temp, auto, ai)
    return msg
...
# 텔레그램 I/O
# ======================================================================
CASH_KEYS = ("현금", "예수금", "cash", "mmf", "cma", "달러", "usd")
...
def save_portfolio(alloc):
    import datetime
    _write_json(PORTFOLIO_FILE,
                {"date": datetime.date.today().isoformat(), "alloc": alloc})


def cash_target_band(temp, auto, ai):
    """온도 → 목표 현금 밴드 (백테스트 유형·피크아웃 수정자 반영)."""
    if temp < 20: lo, hi = 10, 25
    elif temp < 40: lo, hi = 20, 35
    elif temp < 60: lo, hi = 30, 40
    elif temp < 80: lo, hi = 40, 55
    else: lo, hi = 55, 70
    notes = []
    dt = classify_downturn(auto, ai) or ""
    m = auto.get("meta", {})
    if "침식형" in dt:
        lo, hi = max(lo, 45), max(hi, 60)
        notes.append("침식형 수정자: 역발상 무효 — 온도 무관 현금 상향, 신규매수 중단")
    elif "혼합형" in dt or "청산형" in dt:
        lo = (lo + hi) // 2
        notes.append("청산 미완 수정자: 밴드 상단 운용, 분할 1차까지만")
    elif "이벤트 쇼크형" in dt and m.get("rv_peakout"):
        hi = (lo + hi) // 2
        notes.append("이벤트형+피크아웃 확인: 밴드 하단(적극 투입) 운용 가능")
    if temp < 20 and m.get("rv_peakout") is False:
        lo = max(lo, 20)
        notes.append("피크아웃 미확인: 극단 공포라도 현금 하한 20% 유지 (2008 교훈)")
    return lo, hi, notes
...
def build_portfolio_guide(temp, auto, ai):
    ...
    lines = ["💼 *포트폴리오·현금 가이드* (트레이딩 계좌 기준·규칙 기반 참고용)",
    ...
    # 집중도 점검
    eq = sorted(((k, v) for k, v in alloc.items() if k.lower() not in CASH_KEYS),
                key=lambda x: -x[1])
    if eq and eq[0][1] >= 40:
        lines.append(f"⚠️ 집중도: {eq[0][0]} {eq[0][1]:.0f}% — 단일 종목 40%↑")
...
def build_full(auto, ai, answers):
    """최종 결과 1통 (v10): 정량 지표 상세 + AI 조사 + 온도/판정/유형까지 통합."""
    return build_interim(auto, ai) + "\n\n" + build_final(auto, ai, answers)


def send(chat_id, text):
    if len(text) > 3900:  # 텔레그램 4096자 제한 안전 분할
        for i in range(0, len(text), 3900):
            if not send(chat_id, text[i:i + 3900]):
                return False
        return True
    try:
...
def drain_updates():
    """대기 시작 전 밀린 메시지 소진 (v13): 오래된 텍스트의 답장 오인은 방지하되,
    숫자 답변(어제 설문의 늦은 답장)은 버리지 않고 last_answers로 회수 →
    오늘 진단의 잠정 결과에 자동 반영."""
    ...
    if salvaged:
        ...
        _write_json(ANSWERS_FILE,
                    {"date": datetime.date.today().isoformat(), "answers": salvaged})
        print(f"[늦은 답장 회수] {salvaged} → 오늘 잠정 진단에 반영")
    return offset
...
def wait_for_answers(chat_id, auto, ai, minutes):
    ...
    while time.time() < deadline:
        ...
        for upd in r.get("result", []):
            ...
            if _handle_portfolio_cmd(chat_id, _text):
                continue  # 포트 등록 후 계속 답장 대기
            answers = parse_answers(_text, n)
            if answers:
                import datetime
                _write_json(ANSWERS_FILE,
                            {"date": datetime.date.today().isoformat(), "answers": answers})
                send(chat_id, build_full(auto, ai, answers))
...
def run_diagnosis(chat_id, header=None):
    """전체 진단 (v10): ①설문 4문항 선발송 → ②지표 수집(2~3분, 답변 작성 시간) →
    ③답장 대기 → ④정량 지표 상세를 포함한 최종 결과 1통 발송."""
    if header:
        send(chat_id, header)
    drain_updates()
    if not send(chat_id, build_questions()
                + "\n\n⏳ 답하시는 동안 정량 지표를 수집합니다 (2~3분). "
                  "답변과 지표를 종합한 최종 결과를 *한 번에* 보내드립니다"):
        raise RuntimeError("텔레그램 발송 실패 — TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID 확인 "
                           "(봇에게 /start를 먼저 보냈는지도 확인). 헛대기 방지를 위해 즉시 중단")
    auto = collect_auto()
    ai = merge_ai(collect_ai(), [collect_digest_item()])
    save_state(auto, ai)
    wait_min = int(os.environ.get("ANSWER_WAIT_MIN", "5"))
    got = wait_for_answers(chat_id, auto, ai, wait_min) if wait_min > 0 else False
    if not got:
        prev = _read_json(ANSWERS_FILE, None)
        if prev and len(prev.get("answers", [])) == len(USER_QUESTIONS):
            send(chat_id,
                 f"⌛ 답장 미수신 — 지난 응답({prev.get('date', '?')}) 재사용 잠정 결과입니다. "
                 "숫자 4개를 답장해 두면 15:40 실행이 최종 결과로 갱신합니다\n\n"
                 + build_full(auto, ai, prev["answers"]))
        else:
            send(chat_id, "⌛ 답장 미수신 — 숫자 4개를 답장해 두면 "
                          "15:40 실행에서 최종 결과를 발송합니다")


CRASH_THRESHOLD = -0.10  # 급락 판정 임계 (전일 종가 대비 -10%, 서킷브레이커급)
...
def _handle_portfolio_cmd(chat_id, text):
    """포트 등록 명령 처리. 처리했으면 True."""
    ...
    if port == "SUM_ERR":
        ...
        return True
    save_portfolio(port)
    cash = sum(v for k, v in port.items() if k.lower() in CASH_KEYS)
    send(chat_id, "✅ 포트폴리오 등록 완료 (합계 100% 정규화)\n"
                  + " · ".join(f"{k} {v:.0f}%" for k, v in port.items())
                  + f"\n현금 인식: {cash:.0f}% — 다음 진단부터 현금 가이드에 반영")
    return True


def handle_message(chat_id, text):
    text = text.strip()
    if _handle_portfolio_cmd(chat_id, text):
        return
    ...
    if state:
        ...
        if answers:
            ...
            _write_json(ANSWERS_FILE,
                        {"date": datetime.date.today().isoformat(), "answers": answers})
            send(chat_id, build_full(state["auto"], state["ai"], answers))
            os.remove(STATE_FILE)
            return
...
def process_pending(chat_id):
    ...
    if pending_answers:
        ...
        state = load_state()
        if state:
            send(chat_id, build_full(state["auto"], state["ai"], pending_answers))
            os.remove(STATE_FILE)
        else:
...
def weekly():
    """--weekly: 정기 진단 (v13: 평일 11:00 KST 일일 cron. 함수명은 호환 유지)."""
